"""
PPO (Proximal Policy Optimization) model implementation.
This module provides a PPO model for BTC price prediction.
"""
...
class CustomCallback(BaseCallback):
    """
    Custom callback for logging training information during PPO training.
    """
    ...
    def _on_step(self) -> bool:
        ...
        # Log approximate loss
        if hasattr(self.model, "logger") and hasattr(self.model.logger, "name_to_value"):
            if "loss" in self.model.logger.name_to_value:
                self.training_metrics["loss"].append(self.model.logger.name_to_value["loss"])
...
class PPOModel(BaseModel):
    """
    PPO (Proximal Policy Optimization) model for BTC price prediction.
    
    This model uses PPO from Stable-Baselines3 for training and prediction.
    """
    ...
    def __init__(self, model_id: str, model_params: Dict[str, Any] = None):
        ...
        logger.debug(f"PPO model initialized with parameters: {self.model_params}")
    
    def train(self, env: gym.Env, total_timesteps: int = 50000, eval_freq: int = 1000, 
        ...
        # Create PPO model if it doesn't exist
        if self.model is None:
            self.model = PPO(
                env=env,
                **self.model_params
            )
            logger.debug(f"Created new PPO model with parameters: {self.model_params}")
